...
def save_file(config_path, train_path, test_path, train_data, test_data, num_clients, num_classes, statistic):
    ...
    # Save testing data
    test_data_combined = {'x': [], 'y': []}

    # 遍历每个客户端的数据，拼接 x 和 y
    for test_dict in test_data:
        test_data_combined['x'].extend(test_dict['x'])
        test_data_combined['y'].extend(test_dict['y'])

    # 保存为一个 dict，用 'data' 作为 key
    with open(test_path + '0.npz', 'wb') as f:
        np.savez_compressed(f, data=test_data_combined)
